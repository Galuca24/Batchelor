...
function EditBookModal({ open, onClose, audioBook, onAudioBookUpdated }) {
  const [formData, setFormData] = useState({
    ...
    description: '',
    imageUrl: '',
    duration: '', // Adaugă câmpul duration
  ...
  useEffect(() => {
    if (audioBook) {
      setFormData({
        ...
        duration: audioBook.duration || '', // Inițializează duration
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`https://localhost:7115/api/v1/audioBooks/update-audio-book?audioBookId=${audioBook.audioBookId}`, formData);
      console.log("Response data:", response.data); // Adaugă acest log pentru a verifica datele
      if (response.status === 200) {
        toast.success("Book updated successfully!");
       
        onAudioBookUpdated(response.data);
        onClose();
      } else {
