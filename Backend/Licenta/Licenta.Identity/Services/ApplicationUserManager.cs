...
namespace Licenta.Identity.Services
{
    public class ApplicationUserManager : IUserManager
    {
        ...
        public async Task<Result<UserDto>> UserExists(Guid userId)
        {
            var user = await userManager.FindByIdAsync(userId.ToString());
            return user == null ? Result<UserDto>.Failure($"User with id {userId} not found") : Result<UserDto>.Success(MapToUserDto(user));
        }
        ...
        public async Task<Result<List<UserDto>>> FindByRole(string role)
        {
            ...
            return Result<List<UserDto>>.Success(usersInRole);
        }
